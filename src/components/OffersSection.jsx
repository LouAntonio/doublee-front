...
const OffersSection = () => {
	const { data: products = [], isLoading } = useOnSaleProducts();
				...
				<div className="offers-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
					<div className="offers-row" style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
						<div className="offers-deal" style={{
							...
							<div style={{ flex: 1 }}><ProductSkeleton /></div>
						</div>
						<div className="offers-grid-section" style={{
							flex: '1', backgroundColor: '#fff', borderRadius: '6px', padding: '24px',
							...
							<div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
								{skeletonRows.map((_, i) => (
									<div key={`skel-1-${i}`} style={{ flex: '1 1 0', minWidth: 0 }}><ProductSkeleton /></div>
								))}
							</div>
							<div style={{ height: '1px', backgroundColor: '#eee', margin: '4px 0 12px 0' }} />
							<div style={{ display: 'flex', gap: '12px' }}>
								{skeletonRows.map((_, i) => (
									<div key={`skel-2-${i}`} style={{ flex: '1 1 0', minWidth: 0 }}><ProductSkeleton /></div>
								))}
							</div>
						</div>
					</div>
	...
	const dealCandidate = products.reduce((best, product, index) => {
		...
		return discount > best.discount ? { product, index, discount } : best;
	}, null);
              ...
              Oferta do dia
                ...
                Mostrar todas as ofertas
						...
						{firstRow.length > 0 && (
							<div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
								{firstRow.map((offer) => (
									<div key={offer.id} style={cardWrapperStyle}><ProductCard product={offer} /></div>
								))}
