...
const StoreInfoTab = ({ store, onUpdated }) => {
	...
	return (
		<div className="opacity-0 animate-fade-in-up" style={{ animationFillMode: 'forwards' }}>
			<h2 className="text-lg font-bold text-[#1C1917] mb-1 font-display">Informações da Loja</h2>
			<p className="text-sm text-[#78716C] mb-6">Edite os dados públicos e financeiros da sua loja.</p>
			...
			<form onSubmit={handleSubmit} className="max-w-2xl space-y-8">
					...
					<div className="space-y-4">
						<div className="space-y-1.5">
							...
							<input type="text" name="name" value={form.name} onChange={handleChange} required
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="Nome da sua marca" />
						</div>
						<div className="space-y-1.5">
							<label className="text-sm font-medium text-[#1C1917]">Descrição</label>
							<textarea name="description" value={form.description} onChange={handleChange} rows="3"
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="Descreva a sua loja brevemente..." />
						</div>
					</div>
				</div>
				...
				<div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<div className="space-y-1.5">
							<label className="text-sm font-medium text-[#1C1917]">Endereço</label>
							<input type="text" name="location" value={form.location} onChange={handleChange}
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="Bairro, Rua, nº..." />
						</div>
					</div>
				</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div className="space-y-1.5">
							...
							<input type="email" name="email" value={form.email} onChange={handleChange}
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="loja@exemplo.com" />
						</div>
						<div className="space-y-1.5">
							<label className="text-sm font-medium text-[#1C1917]">Telefone Comercial</label>
							<input type="tel" name="phone" value={form.phone} onChange={handleChange}
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="+244 9XX XXX XXX" />
						</div>
						<div className="space-y-1.5 md:col-span-2">
							<label className="text-sm font-medium text-[#1C1917]">Horário de Funcionamento</label>
							<input type="text" name="workingHours" value={form.workingHours} onChange={handleChange}
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="Ex: Seg–Sex 08:00–18:00 | Sáb 08:00–13:00" />
						</div>
					</div>
				</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="space-y-1.5">
								...
								<input type="text" name="iban" value={form.iban} onChange={handleChange} required
									className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="AO06 0006 0000 XXXX XXXX" />
							</div>
							<div className="space-y-1.5">
								...
								<input type="text" name="bankName" value={form.bankName} onChange={handleChange} required
									className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="Nome tal como consta no banco" />
							</div>
						</div>
						<div className="space-y-1.5">
							<label className="text-sm font-medium text-[#1C1917]">Número PayPay <span className="text-[#78716C] font-normal">(opcional)</span></label>
							<input type="text" name="paypayAccount" value={form.paypayAccount} onChange={handleChange}
								className="w-full px-4 py-3 rounded-xl border border-accent/20 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all bg-white" placeholder="Número de telefone associado ao PayPay" />
						</div>
					</div>
				</div>
				...
				<div className="pt-2 flex items-center gap-4">
					<button type="submit" disabled={saving}
						className="px-8 py-3 bg-accent text-white font-semibold rounded-full hover:bg-accent-dark transition-all disabled:opacity-60 cursor-pointer shadow-lg shadow-accent/20">
						{saving ? 'A guardar...' : 'Guardar Alterações'}
					</button>
					{saving && progress && <p className="text-sm text-[#78716C] animate-pulse">{progress}</p>}
				</div>
			</form>
		</div>
	);
