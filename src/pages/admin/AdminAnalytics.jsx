...
const StatSkeleton = () => (
	<div className="bg-white p-6 rounded-2xl shadow-sm border border-accent/10 animate-pulse">
		<div className="flex justify-between items-start mb-4">
			<div className="w-12 h-12 rounded-xl bg-accent/20"></div>
			<div className="h-4 bg-sand rounded w-16"></div>
		</div>
		<div className="h-8 bg-accent/20 rounded w-24 mb-2"></div>
...
const ActivitySkeleton = () => (
	<div className="bg-white p-6 rounded-2xl shadow-sm border border-accent/10 animate-pulse">
		<div className="h-6 bg-accent/20 rounded w-48 mb-6"></div>
		<div className="space-y-4">
			{[1, 2, 3, 4, 5].map(i => (
				<div key={i} className="flex items-center gap-4 py-3 border-b border-accent/10 last:border-0 text-[#78716C]">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				{statCards.map((card, idx) => (
					<div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-accent/10 hover:shadow-md transition-all duration-300 group overflow-hidden relative">
						<div className="absolute top-0 right-0 -mr-4 -mt-4 w-24 h-24 bg-sand/50 rounded-full group-hover:scale-150 transition-transform duration-500"></div>
						<div className="relative z-10">
							<div className="flex justify-between items-start mb-4">
								...
								<span className="text-emerald-500 text-xs font-display font-bold bg-emerald-50 px-2 py-1 rounded-lg">
									{card.trend}
								</span>
							</div>
							<h3 className="text-3xl font-display font-bold text-[#1C1917] tracking-tight">{card.value}</h3>
							<p className="text-sm font-medium text-[#78716C] mt-1">{card.title}</p>
						</div>
					</div>
				))}
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				<div className="bg-white rounded-2xl shadow-sm border border-accent/10 p-6">
					<div className="flex items-center justify-between mb-6">
						<h3 className="text-lg font-display font-bold text-[#1C1917] flex items-center gap-2">
							<svg className="w-5 h-5 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 11-8 0m8 4v4a1 1 0 01-1 1H7a1 1 0 01-1-1v-4m10 0a2 2 0 100-4 2 2 0 000 4zM8 11a2 2 0 100-4 2 2 0 000 4z"></path></svg>
                            Últimas Encomendas
						</h3>
						<button className="text-xs font-display font-bold text-[#78716C] hover:text-accent transition-colors">Ver todas</button>
					</div>
					<div className="space-y-4">
						{stats?.recentOrders?.length > 0 ? (
							stats.recentOrders.map((order, idx) => (
								<div key={idx} className="flex items-center gap-4 py-3 border-b border-accent/10 last:border-0 hover:bg-sand/30 transition-colors px-2 rounded-xl -mx-2">
									<div className="w-10 h-10 rounded-full bg-sand flex items-center justify-center text-[#78716C] font-bold text-xs">
										{order.user?.name?.charAt(0) || 'U'}
									</div>
									<div className="flex-1 min-w-0">
										<p className="text-sm font-bold text-[#1C1917] truncate">{order.user?.name || 'Utilizador Anónimo'}</p>
										<p className="text-xs text-[#78716C] truncate">{new Date(order.createdAt).toLocaleString('pt-AO')}</p>
									</div>
							...
							<div className="py-12 text-center text-[#78716C]">Sem atividade recente.</div>
						)}
					</div>
				</div>
				...
				<div className="bg-white rounded-2xl shadow-sm border border-accent/10 p-6">
					<div className="flex items-center justify-between mb-6">
						...
						<button className="text-xs font-display font-bold text-[#78716C] hover:text-indigo-500 transition-colors">Ver todas</button>
					</div>
					<div className="space-y-4">
						{stats?.recentStores?.length > 0 ? (
							stats.recentStores.map((store, idx) => (
								<div key={idx} className="flex items-center gap-4 py-3 border-b border-accent/10 last:border-0 hover:bg-sand/30 transition-colors px-2 rounded-xl -mx-2">
									...
									<div className="flex-1 min-w-0">
										<p className="text-sm font-bold text-[#1C1917] truncate">{store.name}</p>
										<p className="text-xs text-[#78716C] truncate">{new Date(store.createdAt).toLocaleDateString('pt-AO')}</p>
									</div>
									<div>
							...
							<div className="py-12 text-center text-[#78716C]">Sem novas lojas.</div>
						)}
					</div>
				</div>
			</div>
