	...
	<tr className="animate-pulse border-b border-accent/10 last:border-0">
		...
		<td className="px-6 py-4 whitespace-nowrap text-right">
			<div className="flex justify-end">
				<div className="h-8 w-24 bg-accent/20 rounded-lg"></div>
			</div>
		</td>
	</tr>
);
			...
			<div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-6 rounded-2xl shadow-sm border border-accent/10">
				<div className="max-w-md">
					<h2 className="text-2xl font-display font-bold text-[#1C1917] tracking-tight flex items-center gap-2">
						<svg className="w-6 h-6 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path></svg>
						Gestão de Categorias
					</h2>
					<p className="text-sm text-[#78716C] mt-1 font-body">Crie e organize as categorias de produtos da sua loja.</p>
				</div>
					...
					<div className="flex items-center gap-3 w-full">
						{imagePreview ? (
							<div className="relative w-10 h-10 shrink-0">
								<img src={imagePreview} alt="Preview" className="w-10 h-10 object-cover rounded-lg border border-accent/20" />
							...
							<button type="button" onClick={() => fileInputRef.current?.click()} className="w-10 h-10 shrink-0 flex items-center justify-center bg-sand/50 text-[#78716C] hover:text-accent hover:bg-accent/10 rounded-lg border border-accent/20 transition-colors shadow-sm" title="Adicionar Imagem">
						...
						<div className="flex w-full relative">
							<input
								...
								className="w-full lg:w-64 pl-4 pr-4 py-2.5 rounded-l-xl border border-accent/20 border-r-0 bg-sand/50 text-sm  transition-all outline-none"
							/>
							<button
								type="submit"
								disabled={isCreating}
								className="px-6 py-2.5 whitespace-nowrap border border-transparent text-sm font-display font-semibold rounded-r-xl text-white bg-accent hover:bg-accent-dark transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent disabled:bg-[#78716C]/50"
							>
								{isCreating ? (
					...
					<table className="w-full text-left border-collapse">
						<thead>
							<tr className="bg-sand/50 border-b border-accent/10">
								<th className="px-6 py-4 text-xs font-display font-bold text-[#78716C] uppercase tracking-wider w-16">Imagem</th>
								<th className="px-6 py-4 text-xs font-display font-bold text-[#78716C] uppercase tracking-wider">Identificação / Nome da Categoria</th>
								<th className="px-6 py-4 text-xs font-display font-bold text-[#78716C] uppercase tracking-wider text-right">Controle</th>
							</tr>
						</thead>
									...
									<tr key={category.id} className="hover:bg-sand/50 transition-colors group">
										...
										<td className="px-6 py-4 whitespace-nowrap">
											<div className="flex flex-col">
												<div className="text-sm font-bold text-[#1C1917] group-hover:text-accent-dark transition-colors">{category.name}</div>
												<div className="text-[10px] font-mono text-[#78716C] mt-1 uppercase tracking-wider">ID: {category.id}</div>
											</div>
										</td>
										<td className="px-6 py-4 whitespace-nowrap text-right">
											<button
												onClick={() => handleEditClick(category)}
												className="px-4 py-1.5 rounded-lg text-xs font-display font-bold bg-sand/50 text-[#1C1917] hover:bg-accent/20 hover:text-[#1C1917] border border-accent/20 transition-all shadow-sm"
											>
												Editar
											</button>
										</td>
								...
								<tr>
									<td colSpan="3" className="px-6 py-16 text-center">
										<div className="flex flex-col items-center justify-center text-[#78716C]">
											<svg className="w-12 h-12 mb-3 text-accent/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path></svg>
											<p className="text-base font-semibold text-[#78716C]">Nenhuma categoria encontrada.</p>
